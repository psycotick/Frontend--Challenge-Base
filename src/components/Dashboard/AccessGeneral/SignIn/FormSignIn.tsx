...
import { FormSigInProps } from "./FormSigInProps.types";
...
export function FormSignIn(props: FormSigInProps & { setIsLoggedIn: (isLoggedIn: boolean) => void }) {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            const response = await fetch("http://localhost:5200/apis/user/login", {
                method: "POST",
                headers: {
            ...
            setModalMessage("Error al ingresar. Por favor, intenta nuevamente.");
            setModalOpen(true);
        }
    };
